#generates the transition matrix associated with a process
import birthdeathrates as bdr
        ...
        for k in range(maxi):
          for l in range(maxi):
            if(i == j and k == l and j + 1 == maxi and l+1 == maxi):
                ...
                datvec.append(-bdr.gnm(l+1, j + 1,a,kc));
            elif(i == j and k == l and l+1 == maxi):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(-bdr.gn(l+1, j + 1,a,kc));
            elif(i == j and k == l and j+1 == maxi):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(-bdr.gm(l+1, j + 1,a,kc));
            elif(i == j and k == l):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(-bdr.gg(l+1, j + 1,a,kc));
            elif(i == j and k-1 == l):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(+bdr.bn(l+1, j + 1,a,kc));
            elif(i == j and k+1 == l):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(+bdr.dn(l+1, j + 1,a,kc));
            elif(i-1 == j and k == l):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(+bdr.bm(l+1, j + 1,a,kc));
            elif(i+1 == j and k == l):
                rowvec.append(maxi*i + k);
                colvec.append(maxi*j + l);
                datvec.append(+bdr.dm(l+1, j + 1,a,kc));
    trix = sparse.coo_matrix((datvec,(rowvec,colvec)), shape=(maxi*maxi,maxi*maxi))
    return trix

def sparsepinit(a,kc,maxi):
    n0=int(round(kc/(1.+a)));#or ceil instead of round
    initcond=((n0 - 1)*maxi + n0-1)
