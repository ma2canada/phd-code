...
double bn (int n,int m, double params[4]) {//should probably include "[4]" in all of them so no/less junk gets in
    return params[0]*double(n)*(1.0+params[1] - params[3]*double(n)/params[2]);} // ! should all the inputs be doubles? or at least the output specifically/explicitly?
double dn (int n,int m, double params[4]) {
    return params[0]*n*(params[1] + (1.0-params[3])*double(n)/params[2]);}
...
SparseMatrix<double> makesparsetrix1D(int interspec, double params[4]){    //makes the sparse matrix
//    could also use Triplets (http://eigen.tuxfamily.org/dox/group__TutorialSparse.html#TutorialSparseFilling)
    ...
    mat.reserve(VectorXi::Constant(msize,3));//each column will have at most three elements
...
SparseMatrix<double> makesparsetrix1Dtranspose(int interspec, double params[4]){    //makes the sparse matrix
//    could also use Triplets (http://eigen.tuxfamily.org/dox/group__TutorialSparse.html#TutorialSparseFilling)
    int msize = interspec;
    SparseMatrix<double> mat(msize,msize);
    cout << "[square] matrix size = " << mat.rows() << endl;
...
int main(){
    ...
    ofstream filer; filer.open("kramers-1D-delta1E0-20190527.txt");
...
//    double nK = 8.;  //number of different K values, up to 2^nK
    ...
    for(double aaa=0.0; aaa<1.01; aaa+=0.1){
      for(double jay=0.;jay<nK+0.5;jay++){
        ...
        double qconst = aaa;
        double Kconst = double(8.+8.*jay);//from _ to _*_ took _s (_h)
